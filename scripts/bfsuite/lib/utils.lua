...
function utils.isHeliArmed()
    ...
    govSOURCE = bfsuite.bg.telemetry.getSensorSource("governor")

    if bfsuite.bg.telemetry.getSensorProtocol() == 'lcrsf' then
...
function utils.getCurrentProfile()
    ...
    if (bfsuite.bg.telemetry.getSensorSource("pidProfile") ~= nil and bfsuite.bg.telemetry.getSensorSource("rateProfile") ~= nil) then
        ...
        local p = bfsuite.bg.telemetry.getSensorSource("pidProfile"):value()
        if p ~= nil then
        ...
        local r = bfsuite.bg.telemetry.getSensorSource("rateProfile"):value()
        if r ~= nil then
        ...
        if bfsuite.config.ethosRunningVersion ~= nil then
            ...
            local message = {
                ...
                processReply = function(self, buf)
                    ...
                    if #buf >= 30 then
                        ...
                        buf.offset = 24
                        local activeProfile = bfsuite.bg.msp.mspHelper.readU8(buf)
                        buf.offset = 26
                        local activeRate = bfsuite.bg.msp.mspHelper.readU8(buf)
...
-- ON THE PARAMETERS ON THE bfsuite.pages TABLE
function utils.getFieldValue(f)
    ...
    if f.value ~= nil then
        if f.decimals ~= nil then
            v = bfsuite.utils.round(f.value * bfsuite.utils.decimalInc(f.decimals))
        else
            v = f.value
        end
    else
...
function utils.saveFieldValue(f, value)
    if value ~= nil then
        ...
        if f.decimals ~= nil then
            f.value = value / bfsuite.utils.decimalInc(f.decimals)
        else
            f.value = value
        end
...
function utils.getInlinePositions(f, lPage)
    ...
    local inline_multiplier = bfsuite.app.radio.inlinesize_mult
...
function utils.log(msg)
    ...
    if config.logEnable == true then
        ...
        local f = io.open(config.suiteDir .. "/logs/bfsuite.log", 'a')
