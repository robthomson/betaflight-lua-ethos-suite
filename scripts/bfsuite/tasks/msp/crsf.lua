...
transport.mspPoll = function()
    while true do
        ...
        if cmd == CRSF_FRAMETYPE_MSP_RESP and data[1] == CRSF_ADDRESS_RADIO_TRANSMITTER and data[2] == CRSF_ADDRESS_BETAFLIGHT then
            --[[
                        --bfsuite.utils.log("cmd:0x"..string.format("%X", cmd))
                        --bfsuite.utils.log("  data length: "..string.format("%u", #data))
                        for i=1,#data do
                                --bfsuite.utils.log("  ["..string.format("%u", i).."]:  0x"..string.format("%X", data[i]))
                        end
--]]
            local mspData = {}
