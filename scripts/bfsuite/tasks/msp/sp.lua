...
function transport.sportTelemetryPush(sensorId, frameId, dataId, value)
    ...
    return bfsuite.bg.msp.sensor:pushFrame({physId = sensorId, primId = frameId, appId = dataId, value = value})
end

-- GRAB THE SPORT TELEMETRY FRAME
function transport.sportTelemetryPop()
    -- Pops a received SPORT packet from the queue. Please note that only packets using a data ID within 0x5000 to 0x50FF (frame ID == 0x10), as well as packets with a frame ID equal 0x32 (regardless of the data ID) will be passed to the LUA telemetry receive queue.
    local frame = bfsuite.bg.msp.sensor:popFrame()
...
transport.mspPoll = function()
    while true do
        ...
        if (sensorId == SMARTPORT_REMOTE_SENSOR_ID or sensorId == FPORT_REMOTE_SENSOR_ID) and frameId == REPLY_FRAME_ID then
            -- --bfsuite.utils.log("sensorId:0x"..string.format("%X", sensorId).." frameId:0x"..string.format("%X", frameId).." dataId:0x"..string.format("%X", dataId).." value:0x"..string.format("%X", value))
            local payload = {}
            ...
            -- for i=1,#payload do
            --        --bfsuite.utils.log(  "["..string.format("%u", i).."]:  0x"..string.format("%X", payload[i]))
            -- end
            return payload
