...
if bfsuite.config.tailMode == 1 or bfsuite.config.tailMode == 2 then
...
local function saveData()
    ...
    local payload = bfsuite.app.Page.values
...
local function mixerOn(self)
    ...
    bfsuite.app.audio.playMixerOverideEnable = true
...
local function mixerOff(self)
    ...
    bfsuite.app.audio.playMixerOverideDisable = true
    ...
    for i = 1, 4 do
        ...
        bfsuite.bg.msp.mspHelper.writeU16(message.payload, 2501)
        bfsuite.bg.msp.mspQueue:add(message)
    end
...
local function postLoad(self)
    ...
    if bfsuite.config.tailMode == nil then
        local v = bfsuite.app.Page.values[2]
        bfsuite.config.tailMode = math.floor(v)
        bfsuite.app.triggers.reload = true
        return
    end

    -- existing
    currentRollTrim = bfsuite.app.Page.fields[1].value
    currentPitchTrim = bfsuite.app.Page.fields[2].value
    currentCollectiveTrim = bfsuite.app.Page.fields[3].value

    if bfsuite.config.tailModeActive == 1 or bfsuite.config.tailModeActive == 2 then currentIdleThrottleTrim = bfsuite.app.Page.fields[4].value end

    if bfsuite.config.tailModeActive == 0 then currentYawTrim = bfsuite.app.Page.fields[4].value end
    bfsuite.app.triggers.isReady = true
end
...
local function wakeup(self)
    ...
    if inOverRide == true then
        ...
        if ((now - lastChangeTime) >= settleTime) and bfsuite.bg.msp.mspQueue:isProcessed() then
        ...
        if bfsuite.config.tailMode == 1 or bfsuite.config.tailMode == 2 then
            currentIdleThrottleTrim = bfsuite.app.Page.fields[4].value
            local now = os.clock()
            local settleTime = 0.85
            if ((now - lastChangeTime) >= settleTime) and bfsuite.bg.msp.mspQueue:isProcessed() then
        ...
        if bfsuite.config.tailMode == 0 then
            currentYawTrim = bfsuite.app.Page.fields[4].value
            local now = os.clock()
            local settleTime = 0.85
            if ((now - lastChangeTime) >= settleTime) and bfsuite.bg.msp.mspQueue:isProcessed() then
    ...
    if triggerOverRide == true then
        ...
        if inOverRide == false then
            ...
            bfsuite.app.ui.progressDisplay("Mixer override", "Enabling mixer override...")

            bfsuite.app.Page.mixerOn(self)
            inOverRide = true
        else

            bfsuite.app.audio.playMixerOverideDisable = true

            bfsuite.app.ui.progressDisplay("Mixer override", "Disabling mixer override...")

            bfsuite.app.Page.mixerOff(self)
            inOverRide = false
        end
    end
