...
local function updateStats()
    ...
    fields['memory']:value(bfsuite.utils.round(system.getMemoryUsage().luaRamAvailable / 1000, 2) .. 'kB')
...
local function startTest(duration)
    ...
    testLoader = form.openProgressDialog({
        ...
        wakeup = function()
            ...
            if bfsuite.bg.telemetry.active() == false and startTest == true and system:getVersion().simulation ~= true then
                if testLoader then
                    testLoader:close()
                    testLoader = nil
                end
            ...
            -- do msp query
            if bfsuite.bg.msp.mspQueue:isProcessed() and ((now - RateLimit) >= Rate) then
                RateLimit = now
...
local function openPage(pidx, title, script)
    ...
    bfsuite.app.formNavigationFields['menu'] = form.addButton(line, {
    ...
    -- ACTION BUTTON
    bfsuite.app.formNavigationFields['tool'] = form.addButton(line, {x = x - 5 - buttonWs - buttonWs, y = bfsuite.app.radio.linePaddingTop, w = buttonWs, h = bfsuite.app.radio.navbuttonHeight}, {
        text = "*",
        icon = nil,
        options = FONT_S,
        press = function()
    ...
    fields['rf'] = form.addStaticText(line['rf'], posText, string.upper(bfsuite.bg.msp.protocol.mspProtocol))

    line['memory'] = form.addLine("Memory free")
    fields['memory'] = form.addStaticText(line['memory'], posText, bfsuite.utils.round(system.getMemoryUsage().luaRamAvailable / 1000, 2) .. 'kB')
