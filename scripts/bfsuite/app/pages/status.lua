...
local function getStatus()
    local message = {
        ...
        processReply = function(self, buf)
            -- status.pidCycleTime = bfsuite.bg.msp.mspHelper.readU16(buf)
            -- status.gyroCycleTime = bfsuite.bg.msp.mspHelper.readU16(buf)
            buf.offset = 12
            status.realTimeLoad = bfsuite.bg.msp.mspHelper.readU16(buf)
            -- print("Real-time load: "..tostring(status.realTimeLoad))
            status.cpuLoad = bfsuite.bg.msp.mspHelper.readU16(buf)
            -- print("CPU load: "..tostring(status.cpuLoad))
            buf.offset = 18
            status.armingDisableFlags = bfsuite.bg.msp.mspHelper.readU32(buf)
            buf.offset = 24
            status.profile = bfsuite.bg.msp.mspHelper.readU8(buf)
            -- print("Profile: "..tostring(status.profile))
            buf.offset = 26
            status.rateProfile = bfsuite.bg.msp.mspHelper.readU8(buf)
...
local function getDataflashSummary()
    local message = {
        ...
        processReply = function(self, buf)
            -- bfsuite.print("buf length: "..#buf)
            local flags = bfsuite.bg.msp.mspHelper.readU8(buf)
            summary.ready = (flags & 1) ~= 0
            summary.supported = (flags & 2) ~= 0
            summary.sectors = bfsuite.bg.msp.mspHelper.readU32(buf)
            summary.totalSize = bfsuite.bg.msp.mspHelper.readU32(buf)
            summary.usedSize = bfsuite.bg.msp.mspHelper.readU32(buf)
